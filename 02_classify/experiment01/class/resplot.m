...
for k1 = 1:n
	cl{k1} = load(classifications(k1).name);
end
...
for k2=1:n
	...
	for k3=1:length(fn)
		if(isempty(strfind(fn{k3},'model')))
			...
			if(~isempty(strfind(classifications(k2).name,'norm')))
				expresults(jj).mode = 'norm';
			elseif(~isempty(strfind(classifications(k2).name,'std')))
				expresults(jj).mode = 'std';
			else
